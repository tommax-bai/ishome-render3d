...
| 注册名 | 函数名 | 职责 |
|---|---|---|
| `scene-compile` | `compile_scene` | 输入包 → 三维场景包（毫米制网格 + 材质 + 机位） |
...
用户裁决 2026-09-08，原话——*"三维那面改成毫米吧。我们所有的单位都改成毫米。除了给用户
展示的部分。"* 射程两半，都照做：

- **内部数据一律毫米**：输入包、场景包、底渲回执、算法里的每一个中间量。长度字段一律 `_mm`
  后缀（《开发规范》§4.1 量纲入名），**数据类字段是整数毫米**（`HeightRules`、
  `FurnishingPlacement` 尺寸、`CameraSpec.eye_height_mm` 写成 `int`，让"半毫米"这种没有
  来源的精度进不来）；顶点、深度那些**算出来的**中间量仍是浮点毫米。
  面积不在射程内，仍是 `_sqm`（平方米）——两个量纲各有各的后缀。
- **给人看的仍说米**：本仓唯一的展示面是 CLI 打的那几行，换算只落在 `cli._metres` 一处
  （测试在 `tests/test_display_units.py`，含除不尽与取整两类边界）。业主看的报告句子、
  图上标注、会话话术都不在这个仓，那几处照旧说"2.8 米""138 平米"。

**为什么现在改**：同一件家具在 estate-svc 的资产表里是整数毫米（用户裁决 2026-09-07），
在本仓却是米，靠中间某处换算——换算写在哪儿、有没有写错，没有人能一眼看出来。

**这次只换了单位，没换几何**：改前改后同一份拟真包重渲，`geometry`/`line`/`mask` 三路
11 台相机**逐字节相同**；`depth` 一路 5 台变了，差的每一个像素都正好是 1（量程 65535），
是 16 位量化的末位抖动。对照记在 `tests/test_frozen_views.py` 模块 docstring。
